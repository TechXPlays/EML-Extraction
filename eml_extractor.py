...
def extract_attachments(file: Path, destination: Path) -> None:
    ...
    with file.open('rb') as f:
        ...
        for attachment in attachments:
            ...
            counter = 1
            while True:
                new_filename = generate_new_filename(filename, counter)
                filepath = basepath / new_filename
                if not filepath.exists():
                    break
                counter += 1
            basepath.mkdir(exist_ok=True)
            save_attachment(filepath, attachment.get_payload(decode=True))
            print(f'>> Saved as: {new_filename}')
            
def generate_new_filename(original_filename: str, counter: int) -> str:
